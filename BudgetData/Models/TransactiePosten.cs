...
namespace Syntra.Data.Models
{
	public class TransactiePosten
	{
		...
		public bool Import()
		{
			...
			string data = GetType().GetEmbeddedResource("TransactiePosten.dat");
			...
			if (data.NotEmpty())
			{
				try
				{
					...
					if (jsonData != null)
					{
						if (Count > 0)
						{
							foreach (var tp in jsonData)
							{
								var item = Members.Where(t => t.ID == tp.ID).FirstOrDefault();
								if (item != null)
								{
									item.HoofdCat = tp.HoofdCat !=null ? tp.HoofdCat : item.HoofdCat;
									item.SubCat = tp.SubCat != null ? tp.SubCat : item.SubCat;
									item.Debet = tp.Debet > 0 ? tp.Debet: item.Debet;
									item.Credit = tp.Credit > 0 ? tp.Credit : item.Credit;
